...
# Wrapper script to seed data (embedded python)
...
# Handle CLI args mapping for the python script (which expects --args)
# Logic: 
#   init [count] -> --user admin [--count count]
#   slug count -> --slug slug --count count

ARGS=""

if [ "$1" == "init" ]; then
    COUNT=${2:-"100"}
    ARGS="--user admin --count $COUNT"
    ...
    SLUG=$1
    COUNT=$2
    ARGS="--slug $SLUG --count $COUNT"
    echo "Seeding $COUNT votes to poll $SLUG..."
fi

# Execute via docker exec -i (interactive for stdin)
# We construct the python command string on the fly
# Note: $ARGS are passed to the python script as arguments

docker exec -i -e PYTHONPATH=/app poll_backend python - $ARGS << 'EOF'
import random
import argparse
import sys
import secrets
